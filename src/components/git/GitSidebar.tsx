...
function statusLabel(status: string, t: (key: string) => string): { short: string; color: string; label: string } {
  const s = status.trim()
  if (s === 'M' || s === 'M ') return { short: 'M', color: 'text-amber', label: t('gitSidebar.modified') }
  if (s === 'A' || s === 'A ') return { short: 'A', color: 'text-mint', label: t('gitSidebar.added') }
  if (s === 'D' || s === 'D ') return { short: 'D', color: 'text-danger', label: t('gitSidebar.deleted') }
  if (s === 'R' || s === 'R ') return { short: 'R', color: 'text-accent-bright', label: t('gitSidebar.renamed') }
  if (s === 'C' || s === 'C ') return { short: 'C', color: 'text-accent-bright', label: t('gitSidebar.copied') }
  if (s.includes('?')) return { short: '?', color: 'text-muted', label: t('gitSidebar.untracked') }
  if (s.includes('U')) return { short: 'U', color: 'text-danger', label: t('gitSidebar.unmerged') }
...
export function GitSidebar({ project, gitStatus, onClose, onRefresh }: Props) {
  ...
  const handleAbortMerge = async () => {
    try {
      ...
      addToast('success', t('gitSidebar.mergeAborted'))
      await refreshAll()
      onRefresh()
      window.dispatchEvent(new CustomEvent('app:refresh-git-status'))
    } catch (e) {
  ...
  const handleSwitchBranch = async (name: string) => {
    ...
    if (ok && prevBranch && prevBranch !== name) {
      pushUndo(
        t('gitSidebar.switchToBranch', { name }),
  ...
  const handleCreateBranch = async () => {
    ...
    if (ok) {
      pushUndo(
        t('gitSidebar.createBranch', { name: branchName }),
  ...
  const handleDiscardChanges = () =>
    doAction('discard', async () => {
      ...
      if (stashResult) {
        ...
        addToast('info', t('gitSidebar.changesSavedToStash', { label: stashLabel }))
      }
  ...
  const handleStageFiles = async () => {
    ...
    try {
      ...
      setStagedFiles(new Set())
      await refreshAll()
      addToast('success', t('gitSidebar.filesStaged', { count: stagedFiles.size }))
    } catch (e) {
  ...
  const handleCommit = async () => {
    ...
    try {
      ...
      showGitSuccess(amendMode ? t('gitSidebar.commitAmended') : t('gitSidebar.committed'), result || t('gitSidebar.changesCommittedSuccessfully'))

      if (!amendMode) {
        pushUndo(
          t('gitSidebar.commitMessage', { msg: msg.length > 30 ? msg.slice(0, 30) + '…' : msg }),
      ...
      if (pushAfterCommit && remoteUrl) {
        try {
          ...
          if (pushResult) showGitSuccess(t('gitSidebar.pushed'), pushResult)
        } catch (e) {
          showGitError(t('gitSidebar.pushFailed', { error: String(e) }))
        }
      }
  ...
  const executePush = async (force: boolean) => {
    await doAction(force ? 'force-push' : 'push', async () => {
      try {
        ...
        if (r) showGitSuccess(force ? t('gitSidebar.forcePushSucceeded') : t('gitSidebar.pushed'), r)
      } catch (e: unknown) {
        const errStr = String(e)
        if (!force && (errStr.toLowerCase().includes('non-fast-forward') || errStr.toLowerCase().includes('[rejected]') || errStr.toLowerCase().includes('failed to push'))) {
          addToast('info', t('gitSidebar.pushRejectedPullingFirst'))
          try {
            await api.gitPull(project.path)
            const retry = await api.gitPush(project.path)
            showGitSuccess(t('gitSidebar.pushSucceededAfterPull'), retry || t('gitSidebar.changesPushedSuccessfully'))
            return
          } catch (pullErr) {
            showGitError(t('gitSidebar.pullThenPushFailed', { error: String(pullErr) }))
            throw pullErr
          }
        }
        throw e
    ...
    <div className="w-[380px] h-full flex flex-col overflow-hidden relative">
      ...
      <div className="flex items-center justify-between px-5 py-4 border-b border-line shrink-0">
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2">
            ...
            <Tooltip content={t('gitSidebar.betaTooltip')} side="bottom">
              <button
          ...
          {isRepo && currentBranch && (
            <p className="text-[11px] font-mono text-muted mt-0.5 truncate">
              {currentBranch.name}
              {changedFiles.length > 0 && <span className="ml-1.5 text-amber">· {changedFiles.length} {t('gitSidebar.uncommitted')}</span>}
            </p>
          )}
        </div>
        <button onClick={onClose} aria-label={t('gitSidebar.closeSidebar')} className="focus-ring cursor-pointer p-1.5 rounded-lg text-muted hover:text-ink hover:bg-raised transition-colors shrink-0">
      ...
      {!isRepo ? (
        <div className="flex-1 flex flex-col items-center justify-center gap-4 p-8 text-center">
          ...
          <p className="text-sm text-muted">{t('gitSidebar.notGitRepo')}</p>
          <button disabled={busyAction === 'init'} onClick={handleInit}
            ...
            {busyAction === 'init' ? t('gitSidebar.initializing') : t('gitSidebar.initializeRepo')}
          </button>
        </div>
      ) : (
        <>
          ...
          <div className="flex items-center gap-2 px-5 py-3 border-b border-line shrink-0 flex-wrap">
            ...
            {mergeActive && !showMergeConflicts && (
              <div className="w-full flex items-center gap-2 px-3 py-2 rounded-lg bg-danger/10 border border-danger/30 mb-1">
                ...
                <span className="flex-1 text-[11px] text-danger font-medium">{t('gitSidebar.mergeInProgress')}</span>
                <button
            ...
            <button disabled={busyAction !== null || showMergeConflicts}
              ...
              title={showMergeConflicts ? t('gitSidebar.resolveConflictsFirst') : t('gitSidebar.pullLatestChanges')}>
              <IconCloudArrowDown className="w-3 h-3" />{busyAction === 'pull' ? t('gitSidebar.loading') : showMergeConflicts ? t('gitSidebar.conflicts') : t('gitSidebar.pull')}
            </button>
            <button onClick={handlePushAction} disabled={busyAction !== null}
              ...
              <IconArrowUpDown className="w-3 h-3" />{busyAction === 'push' ? t('gitSidebar.loading') : t('gitSidebar.push')}
            </button>
            <button disabled={busyAction !== null}
              ...
              <IconBomb className="w-3 h-3" />{busyAction === 'force-push' ? t('gitSidebar.loading') : t('gitSidebar.force')}
            </button>
            <button disabled={busyAction !== null}
              ...
              <IconRefresh className={`w-3 h-3 ${busyAction === 'fetch' ? 'animate-spin' : ''}`} />{busyAction === 'fetch' ? t('gitSidebar.loading') : t('gitSidebar.fetch')}
            </button>
            <button onClick={() => api.openTerminal(project.path)} title={t('gitSidebar.openInTerminal')}
          ...
          <div className="flex-1 overflow-y-auto">
            ...
            <div className="px-5 pt-4 pb-2 border-b border-line">
              <div className="flex items-center justify-between mb-2">
                <h4 className="text-[10px] font-semibold uppercase tracking-wider text-muted/60">{t('gitSidebar.remote')}</h4>
                <button onClick={() => setShowRemoteInput((v) => !v)}
                  ...
                  {remoteUrl ? t('gitSidebar.change') : t('gitSidebar.add')}
                </button>
              </div>
              {showRemoteInput ? (
                <div className="flex items-center gap-1.5 mb-1">
                  <input type="text" value={remoteInput} onChange={(e) => setRemoteInput(e.target.value)}
                    placeholder={remoteUrl ? t('gitSidebar.newRemoteUrl') : 'https://github.com/user/repo.git'}
                ...
                <div className="flex items-center gap-1.5">
                  ...
                  <button onClick={() => setShowRemoveRemoteConfirm(true)} disabled={busyAction !== null} title={t('gitSidebar.removeRemote')}
                    className="focus-ring cursor-pointer p-1 rounded text-muted hover:text-danger transition-colors">
                    <IconTrash className="w-3 h-3" />
                  </button>
                </div>
              ) : (
                <p className="text-[11px] text-muted/60 py-1">{t('gitSidebar.noRemoteConfigured')}</p>
              )}
            </div>
            ...
            <div className="px-5 pt-4 pb-2 border-b border-line">
              <div className="flex items-center justify-between mb-2">
                <h4 className="text-[10px] font-semibold uppercase tracking-wider text-muted/60">{t('gitSidebar.branches')}</h4>
                <button onClick={() => setShowCreateBranch((v) => !v)}
                  className="focus-ring cursor-pointer text-[10px] text-accent-bright hover:underline transition-colors">{t('gitSidebar.new')}</button>
              </div>
              {showCreateBranch && (
                <div className="flex items-center gap-1.5 mb-2">
                  <input type="text" value={newBranchName} onChange={(e) => setNewBranchName(e.target.value)}
                    placeholder={t('gitSidebar.branchName')}
              ...
              {branchesLoading ? (
                <div className="flex items-center gap-2 py-2"><IconRefresh className="w-3 h-3 animate-spin text-muted" /><span className="text-[11px] text-muted">{t('gitSidebar.loadingBranches')}</span></div>
              ) : branches.length === 0 ? (
                <p className="text-[11px] text-muted/60 py-2">{t('gitSidebar.noBranchesFound')}</p>
              ) : (
                  ...
                  {branches.map((b) => (
                    <div key={b.name}
                      ...
                      {b.is_current ? (
                        ...
                        >
                          {switchedTo === b.name ? t('gitSidebar.switched') : t('gitSidebar.active')}
                        </span>
                      ) : (
                        <div className="flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity">
                          <button onClick={() => handleSwitchBranch(b.name)} disabled={busyAction !== null}
                            className="focus-ring cursor-pointer p-1 rounded text-muted hover:text-accent-bright hover:bg-raised disabled:opacity-40 transition-colors" title={t('gitSidebar.switchToBranch', { name: b.name })}>
                            <IconCheck className="w-3 h-3" />
                          </button>
                          {b.name !== 'main' && b.name !== 'master' && (
                            <button onClick={() => setConfirmBranchDelete(b.name)} disabled={busyAction !== null}
                              className="focus-ring cursor-pointer p-1 rounded text-muted hover:text-danger hover:bg-raised disabled:opacity-40 transition-colors" title={t('gitSidebar.deleteBranch', { name: b.name })}>
                              <IconTrash className="w-3 h-3" />
                            </button>
                          )}
                        </div>
            ...
            <div className="px-5 pt-4 pb-2 border-b border-line">
              <div className="flex items-center justify-between mb-2">
                <h4 className="text-[10px] font-semibold uppercase tracking-wider text-muted/60">{t('gitSidebar.changes')}</h4>
                {(unstagedFiles.length > 0 || stagedFiles.size > 0) && (
                  <div className="flex items-center gap-2">
                    {/* Select / Deselect All */}
                    {stagedFiles.size < unstagedFiles.length && (
                      <button onClick={selectAllUnstaged}
                        className="focus-ring cursor-pointer text-[10px] text-accent-bright hover:underline transition-colors">{t('gitSidebar.selectAll')}</button>
                    )}
                    {(stagedFiles.size > 1 || (stagedFiles.size > 0 && unstagedFiles.length === 0)) && (
                      <button onClick={deselectAll}
                        className="focus-ring cursor-pointer text-[10px] text-muted hover:text-ink hover:underline transition-colors">{t('gitSidebar.deselectAll')}</button>
                    )}
                    <span className="text-muted/30">·</span>
                    {(stagedFiles.size > 1 || (stagedFiles.size > 0 && unstagedFiles.length === 0)) && (
                      <button onClick={() => setShowDiscardConfirm(true)} disabled={busyAction !== null}
                        className="focus-ring cursor-pointer text-[10px] text-danger hover:underline disabled:opacity-40 disabled:cursor-not-allowed transition-colors">{t('gitSidebar.discardAll')}</button>
                    )}
                    {stagedFiles.size > 0 && (
                      <button onClick={handleStageFiles} disabled={busyAction !== null}
                        className="focus-ring cursor-pointer text-[10px] text-mint hover:underline disabled:opacity-40 disabled:cursor-not-allowed transition-colors">{t('gitSidebar.stageCount', { count: stagedFiles.size })}</button>
                    )}
                  </div>
                )}
              </div>
              ...
              {changesLoading ? (
                <div className="flex items-center gap-2 py-2"><IconRefresh className="w-3 h-3 animate-spin text-muted" /><span className="text-[11px] text-muted">{t('gitSidebar.checkingChanges')}</span></div>
              ) : changedFiles.length === 0 ? (
                <p className="text-[11px] text-muted/60 py-2">{t('gitSidebar.workingTreeClean')}</p>
              ) : (
                  ...
                  {gitStagedFiles.length > 0 && (
                    <>
                      <div className="flex items-center gap-1.5 px-1 py-1 mt-1">
                        ...
                        <span className="text-[9px] font-semibold uppercase text-mint tracking-wider">{t('gitSidebar.stagedCount', { count: gitStagedFiles.length })}</span>
                      </div>
                  ...
                  {pendingStageFiles.length > 0 && (
                    <>
                      <div className="flex items-center gap-1.5 px-1 py-1 mt-1">
                        ...
                        <span className="text-[9px] font-semibold uppercase text-accent-bright tracking-wider">{t('gitSidebar.pendingStageCount', { count: pendingStageFiles.length })}</span>
                      </div>
                      ...
                      <div className="flex items-center justify-between gap-1.5 px-1 py-1 mt-1">
                        <div className="flex items-center gap-1.5">
                          ...
                          <span className="text-[9px] font-semibold uppercase text-amber tracking-wider">
                            {unstagedFiles.length === changedFiles.length - gitStagedFiles.length - pendingStageFiles.length ? t('gitSidebar.unstaged') : t('gitSidebar.unstagedCount', { count: unstagedFiles.length })}
                          </span>
                        </div>
                      </div>
              ...
              {changedFiles.length > 0 && (
                <div className="mt-3 flex flex-col gap-2 pb-1">
                  <textarea
                    ...
                    placeholder={t('gitSidebar.commitMessagePlaceholder')}
                    rows={2}
                  ...
                  <div className="flex items-center gap-3">
                    <label className="flex items-center gap-1.5 cursor-pointer select-none" onClick={() => setAmendMode((v) => !v)}>
                      ...
                      <span className="text-[10px] text-muted hover:text-ink transition-colors">{t('gitSidebar.amend')}</span>
                    </label>
                    {remoteUrl && (
                      <label className="flex items-center gap-1.5 cursor-pointer select-none" onClick={() => setPushAfterCommit((v) => !v)}>
                        ...
                        <span className="text-[10px] text-muted hover:text-ink transition-colors">{t('gitSidebar.pushAfter')}</span>
                      </label>
                    )}
                    ...
                    >
                      <IconCheck className="w-3 h-3" />
                      {busyAction === 'commit'
                        ? (pushAfterCommit ? t('gitSidebar.commPushing') : t('gitSidebar.committing'))
                        : (amendMode ? t('gitSidebar.amend') : t('gitSidebar.commit'))}
                    </button>
                  </div>
                </div>
              )}
            ...
            {(undoHistory.length > 0 || redoHistory.length > 0) && (
              <div className="px-5 pt-4 pb-3 border-b border-line">
                <div className="flex items-center justify-between mb-2">
                  <h4 className="text-[10px] font-semibold uppercase tracking-wider text-muted/60">{t('gitSidebar.actions')}</h4>
                </div>
                <div className="flex flex-col gap-0.5">
                  {redoHistory.slice(0, 3).map((entry) => (
                    <button key={`redo-${entry.id}`}
                      ...
                      <span className="text-[11px] text-muted truncate flex-1">{t('gitSidebar.redo')} {entry.label}</span>
                    </button>
                  ))}
                  {undoHistory.slice(0, 5).map((entry) => (
                    <button key={`undo-${entry.id}`}
                      ...
                      <span className="text-[9px] text-accent-bright font-semibold uppercase shrink-0">{t('gitSidebar.undo')}</span>
                    </button>
                  ))}
                </div>
              </div>
            ...
            <div className="px-5 pt-4 pb-2 border-b border-line">
              <div className="flex items-center justify-between mb-2">
                <h4 className="text-[10px] font-semibold uppercase tracking-wider text-muted/60">{t('gitSidebar.stashes')}</h4>
              </div>
              {stashesLoading ? (
                <div className="flex items-center gap-2 py-2"><IconRefresh className="w-3 h-3 animate-spin text-muted" /><span className="text-[11px] text-muted">{t('gitSidebar.loadingStashes')}</span></div>
              ) : stashes.length === 0 ? (
                <p className="text-[11px] text-muted/60 py-2">{t('gitSidebar.noStashes')}</p>
              ) : (
                    ...
                    <div key={s.index} className="group flex items-center gap-2 px-2.5 py-1.5 rounded-md hover:bg-raised transition-colors">
                      ...
                      <div className="flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button onClick={() => handleStashApply(s.index)} disabled={busyAction !== null}
                          className="focus-ring cursor-pointer p-1 rounded text-muted hover:text-accent-bright transition-colors" title={t('gitSidebar.applyStash')}><IconCheck className="w-3 h-3" /></button>
                        <button onClick={() => handleStashDrop(s.index)} disabled={busyAction !== null}
                          className="focus-ring cursor-pointer p-1 rounded text-muted hover:text-danger transition-colors" title={t('gitSidebar.dropStash')}><IconTrash className="w-3 h-3" /></button>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <div className="px-5 pt-4 pb-5">
              <h4 className="text-[10px] font-semibold uppercase tracking-wider text-muted/60 mb-3">{t('gitSidebar.recentCommits')}</h4>
              {logLoading ? (
                <div className="flex items-center justify-center py-6"><IconRefresh className="w-4 h-4 animate-spin text-muted" /></div>
              ) : logEntries.length === 0 ? (
                <div className="border border-dashed border-line rounded-xl py-6 text-center"><p className="text-xs text-muted">{t('gitSidebar.noCommitsFound')}</p></div>
              ) : (
      ...
      <AnimatePresence>
        {showDiscardConfirm && (
          <ConfirmDialog
            title={t('gitSidebar.discardAllChanges')}
            description={t('gitSidebar.discardAllChangesDesc')}
            confirmLabel={t('gitSidebar.discard')}
            variant="danger"
      ...
      <AnimatePresence>
        {confirmBranchDelete && (
          <ConfirmDialog
            title={t('gitSidebar.deleteBranchConfirm', { name: confirmBranchDelete })}
            description={t('gitSidebar.deleteBranchDesc')}
            confirmLabel={t('gitSidebar.delete')}
            variant="danger"
      ...
      <AnimatePresence>
        {showRemoveRemoteConfirm && (
          <ConfirmDialog
            title={t('gitSidebar.removeRemoteConfirm')}
            description={t('gitSidebar.removeRemoteDesc')}
            confirmLabel={t('gitSidebar.remove')}
            variant="danger"
      ...
      <AnimatePresence>
        {showStashPushConfirm && (
          <ConfirmDialog
            title={t('gitSidebar.stashAllChanges')}
            description={t('gitSidebar.stashAllChangesDesc')}
            confirmLabel={t('gitSidebar.stash')}
      ...
      <AnimatePresence>
        {showPushConfirm && (
          <ConfirmDialog
            title={t('gitSidebar.pushWithUncommitted')}
            description={t('gitSidebar.pushWithUncommittedDesc')}
            confirmLabel={t('gitSidebar.pushAnyway')}
            variant="default"
      ...
      <AnimatePresence>
        {showForcePushConfirm && (
          <ConfirmDialog
            title={t('gitSidebar.forcePushConfirm')}
            description={t('gitSidebar.forcePushDesc')}
            confirmLabel={t('gitSidebar.forcePush')}
            variant="danger"
